...
def generate_briefing(df_coll: pd.DataFrame, df_311: pd.DataFrame, 
    ...
    trends_yoy = analyze_trends_yoy(df_coll, 'DT_ACCDN')
    trends_mom = analyze_trends_mom(df_coll, 'DT_ACCDN', last_n_months=6)
    briefing_data['trends_yoy'] = trends_yoy
    briefing_data['trends_mom'] = trends_mom
    
    # 4. Signaux faibles
    logger.info("  → Détection des signaux faibles...")
    weak_signals = detect_weak_signals(df_311, 'DDS_DATE_CREATION', 
    ...
    if briefing_type == 'weekly':
        ...
        df_coll_recent['DT_ACCDN'] = pd.to_datetime(df_coll_recent['DT_ACCDN'], errors='coerce')
        df_daily = df_coll_recent.groupby(df_coll_recent['DT_ACCDN'].dt.date).size()
        anomalies = detect_anomalies_zscore(df_daily.reset_index(name='count'), 
                                           'DT_ACCDN', 'count', threshold=2.5)
