...
def get_top_hotspots_collisions(df_coll: pd.DataFrame, n_clusters: int = 5, 
    ...
    if date_start and date_end:
        ...
        df_filtered = df[(df['DT_ACCDN'] >= date_start) & (df['DT_ACCDN'] <= date_end)]
    ...
    for cluster_id in range(kmeans.n_clusters):
        ...
        hotspots.append({
            ...
            'observations': cluster_data[['DT_ACCDN', 'LOC_LAT', 'LOC_LONG', 'GRAVITE']].to_dict('records')[:3]
        })
...
def get_top_hotspots_311(df_311: pd.DataFrame, motif_col: str = 'ACTI_NOM', 
    ...
    if date_start and date_end:
        ...
        df_filtered = df[(df['DDS_DATE_CREATION'] >= date_start) & (df['DDS_DATE_CREATION'] <= date_end)]
        # Si aucune donnée dans la période, utiliser toutes les données
        if len(df_filtered) > 0:
            df = df_filtered
        else:
